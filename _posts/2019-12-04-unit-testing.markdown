...
You most likely heard about [Testing Pyramid](https://martinfowler.com/bliki/TestPyramid.html). The idea is that unit tests are cheap and UI and integration tests are not, and that you need to have more of the former. I partially agree with this, but I would like to point out two things.

> **1.** UI and integration tests are expensive for a reason. It's known that the root cause of 99%[^3] of the defects in iOS apps is UIKit. And you can't test it with just unit tests. Yes, unit tests are cheap, but they are no substitute for UI tests. Make sure you have both! I think that for complex systems integration tests are far more important than unit tests.
...
MVVM and MVP allow you to test ViewModels or Presenters in isolation. Some might say that this is the whole point of these architectures, otherwise you are better off sticking with Apple MVC. However, there is a problem with these types of unit tests. I think most of them are a waste.
...
Now, am I saying that you should *not* test Controllers and Views? Absolutely not. This is where UI and integration tests come in. As for unit testing, I would suggest investing in a good software design instead. Make sure ViewModels themselves read like a specification, you will never need to write unit tests for them again.

There is also an unintendend consequence of creating these tests – they become a huge barrier to refactoring[^7]. Eventually, thetests erode without significant investment in their maintanence.
...
- Treat test code as well as you treat your app's code. Apply all of the good engineering principles that you know. Avoid duplication, refactor, strive for readability.
- Follow the [FIRST principles](https://pragprog.com/magazines/2012-01/unit-tests-are-first).
...
TODO: https://martinfowler.com/testing/

## Final Thoughts

Optimize confidence. If the tests you are adding doesn't add confidence, maybe it's not worth writing this code.

- Even if you are doing some of it wrong, the risks are not high – you are not shipping this code in production.

Unit tests are often correlated with a good software design. But, you absolutely can have a well designed system with zero defects without having a single unit test.

TODO: add footnote, how to measure quality. I have spoken.


If you are still on the fence regarding unit testing, it's never too late to give it a try. Don't put too much pressure on yourself, don't try to force yourself to achieve 100% test coverage. You don't have to jump on the TDD/BDD bandwagon to start unit testing. Focus on value over volume, and you'll be surprised how effective unit testing can be.

The bottom line is if the business comes to you and ask how confident you are to make a certain type of change or a bug fix, if you are doing test automation right, you are going to be confident every time.
